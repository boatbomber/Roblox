...
--[=[
	...
	module:Sanitize()
	takes a table and returns a JSON compatible copy

	module:Desanitize()
	takes a sanitized table and returns a normalized copy

	module:Serialize()
	takes a table, sanitizes it, encodes as JSON, and returns the string

	module:Deserialize()
	takes a JSON string, decodes it, desanitizes it, then returns the desanitized table
	...
	Currently unsupported datatypes: --TODO (These aren't Properties so they weren't MVP for my serializer)
...
function module:Sanitize(Object: any, Recursive: boolean?)
	if TypeHandlers[typeof(Object)] then
		return TypeHandlers[typeof(Object)].Sanitize(Object)
	end
...
function module:Desanitize(SanitizedObject: {__sanitize: string?})
	...
	local RegularTable = table.create(#SanitizedObject)
...
function module:Serialize(Object: any)
	local Sanitized = self:Sanitize(Object)
	local Serialized = HttpService:JSONEncode(Sanitized)

	return Serialized
end

function module:Deserialize(Serialized: string)
	local Sanitized = HttpService:JSONDecode(Serialized)
	local Desanitized = self:Desanitize(Sanitized)
